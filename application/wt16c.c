...
{
    float last_pitch_angle = wt61c_data.angle[1].f;
    float last_roll_angle = wt61c_data.angle[0].f;
                    ...
                    switch (wt61c_data_buff[1]) //根据帧头数据
                    {
                        ...
                        case WT61C_ANGLE_ID:
													for(int8_t i = 0; i < 3; i++)
													{
                                                        ...
                                                        roll_round ++;
                                                    // euler_angle.real_pitch = wt61c_data.angle[0].f + roll_round * 360;//陀螺仪水平放置
                                                    euler_angle.real_roll = wt61c_data.angle[0].f + roll_round * 360;//陀螺仪垂直放置
                                                    
                                                    if(wt61c_data.angle[1].f - last_roll_angle > 300.0f)
                                                        roll_round --;
                                                    else if(wt61c_data.angle[1].f - last_roll_angle < -300.0f)
                                                        roll_round ++;
                                                    // euler_angle.real_roll = wt61c_data.angle[1].f + roll_round * 360;//陀螺仪水平放置
                                                    euler_angle.real_pitch = wt61c_data.angle[1].f + roll_round * 360;//陀螺仪垂直放置
                        ...
                        default:
                            break;
                                                ...
                                                //  euler_angle.real_pitch = wt61c_data.angle[1].f;
