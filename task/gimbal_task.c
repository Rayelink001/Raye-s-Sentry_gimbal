...
void gimbal_data_feedback(void) 
{
    ...
    if(gimbal_control_data.yaw_motor.imu_pos_set - gimbal_control_data.yaw_motor.imu_pos_ref > 180)
    {
        gimbal_control_data.yaw_motor.imu_pos_set += 359;
    }else if(gimbal_control_data.yaw_motor.imu_pos_set - gimbal_control_data.yaw_motor.imu_pos_ref < -180)
    {
        gimbal_control_data.yaw_motor.imu_pos_set -= 359;
    }
}
...
void gimbal_control_set(void)
{
    ...
    switch (gimbal_control_data.control_mode)
    {
        ...
        case GIMBAL_CONTROL_MODE_VISION_IMU:
        {
            ...
            if(add_yaw_angle <= -9.0f)
            {
                add_yaw_angle = -9.0f ;
            }
            else if(add_yaw_angle >= 9.0f)
            {
                add_yaw_angle = 9.0f ;
            }
            if(add_pitch_angle <= -7.0f)
            {
                add_pitch_angle = -7.0f ;
            }
            else if(add_pitch_angle >= 7.0f)
            {
                add_pitch_angle = 7.0f ;
            }
            if(gimbal_control_data.vision_data->is_find_target == 1)
            {
            ...
            gimbal_control_data.yaw_motor.offset_angle = gimbal_control_data.yaw_motor.imu_pos_set;
            }
            else if (gimbal_control_data.vision_data->is_find_target == 0){
                gimbal_control_data.yaw_motor.imu_pos_set =gimbal_control_data.yaw_motor.offset_angle;
            }
            gimbal_control_data.pitch_motor.imu_pos_set =  add_pitch_angle;
...
void gimbal_motor_vis_gyro_imu_loop(gimbal_motor_t *motor, pid_data_t *pid_speed, pid_data_t *pid_pos)
{
     #if yaw_pid_speed_debug_mode || pitch_pid_speed_debug_mode //速度环debug   
        motor->current_set = pid_calc(pid_speed, motor->imu_speed_ref, debug_set_speed);    //角速度转电压
    #else //串级角度环
