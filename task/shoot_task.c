...
{  
    ...
    while (1)
    {
        ...
        if(shoot_control_data.shoot_mode == SHOOT_READY_TO_SHOOT || shoot_control_data.shoot_mode == SHOOT_READY_TO_SINGLE_SHOOT)
        {
            //退出发射准备状态的时间
        ...
        if(shoot_control_data.shoot_mode == SHOOT_REARY_TO_LOCK || shoot_control_data.shoot_mode == SHOOT_LOCK) 
        {
			...
			shoot_control_data.led_state->shoot_state.state =  LED_STATE_GREEN;
        }
        else
        {
            //操作手指示灯提示
            ...
            shoot_control_data.led_state->shoot_state.state =  LED_STATE_YELLOW;
        }
...
void shoot_init(void)//发射机构初始化
{
    ...
    shoot_control_data.bloack_flag = 0;             //初始化堵转标志位
		...
		shoot_control_data.led_state = get_led_state();		//绑定指示灯数据结构体
...
//获取发射状态机
shoot_control_mode_e get_shoot_control_mode(void)
{
    return shoot_control_data.shoot_mode;
}
